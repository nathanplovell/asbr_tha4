...
% config_a and config_b, respectively. This code is also an ATTEMPT to incldude
...
function [t_vec, t_vec_mat] = IK_tool_position_VW(robot, config_a, config_b, plot)
    ...
    while physical_err > 0.003
        ...
        n = [0; 0; 1]; % Unit vector normal to virtual wall
        d_wall = 0.8; % Virtual wall is an x-y plane located at z = 0.8 m
        A = -(n'*C); % Negative signs to ensure inequality is in the right direction
        b = -(d_wall - n'*t);
        ...
        [dq_vec,resnorm] = lsqlin(C,d,A,b,[],[],lb,ub,x0,options);
