...
function [t_vec, t_vec_mat] = IK_tool_position(robot, config_a, config_b, plot)
    ...
    while (Vb_v_err > v_err) || (Vb_w_err > w_err)
        ...
        % re-calculating the body Jacobian for each iteration
        Jb = J_body(robot, t_vec'); 
        ...
        p_goal = Tsd(1:3,4);
        %t = M*p_tip 
        t = M(1:3,4); % Have to figure out how I'm dealing with the tip
        J_a = Jb(1:3,:);
        J_e = Jb(4:6,:);
        C = vec2SkewSym(-t)*J_a + J_e;
        d = t - p_goal; % Do I have to change func to only accept p_goal or can I extract from config_b

        % Constraints
        lb = panda.limits(:,1); % Lower joint limits
        ub = panda.limits(:,2); % Upper joint limits
